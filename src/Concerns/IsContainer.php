...
trait IsContainer
{
    ...
    /**
     * If you are using IsContainer only, then you can safely
    ...
    {
        if ($this->disposition(HasFactory::class)) {
        ...
        if ($this->dispositionOf($obj, CanInitialize::class)) {
            if (!$obj->_initialized) {
                $obj->init();
            }
            if (!$obj->_initialized) {
    ...
    protected function _add_Container($element, $args = [])
    {
        ...
        // Carry on reference to application if we have HasAppScope traits set
        if ($this->hasDispositionMatch($element, HasAppScope::class)) {
            $element->app = $this->app;
        }

        // If element is not trackable, then we don't need to do anything with it
        if (!$this->dispositionOf($element, IsTrackable::class)) {
            return $element;
        }
