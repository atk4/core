 ...
 * into your object, but unlike "IsContainer" you can use
...
trait HasCollections
{
    ...
    public function _addIntoCollection(string $name, object $object, string $collection)
    {
        ...
        // Carry on reference to application if we have HasAppScope set in both
        if ($this->hasDispositionMatch($object, HasAppScope::class)) {
            $object->app = $this->app;
        }

        // Calculate long "name" but only if both are trackables
        if ($this->dispositionOf($object, IsTrackable::class)) {
            $object->short_name = $name;
            $object->owner = $this;
            if ($this->disposition(IsTrackable::class)) {
                $object->name = $this->_shorten_ml($this->name.'-'.$collection.'_'.$name);
            }
        }

        if ($this->dispositionOf($object, CanInitialize::class)) {
     ...
     * Identical implementation to HasContainer::_shorten.
     *
